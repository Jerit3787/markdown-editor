...
- Produces:
  ...
  - `withdrawSuggestion(doc: Y.Doc, id: string): void` (alias for the "make my own proposal go away, don't judge it" case — for an insert this deletes the inserted text like a reject; for a delete this just drops the entry like an accept-of-nothing... precisely: `withdrawSuggestion` calls `resolveSuggestion(doc, id, entry.kind === "insert" ? "reject" : "accept")` internally, i.e. always resulting in "the suggested change never happened")
