...
// The refresh token itself lasts ~6 months idle (Google's rule); give the
// cookie a 90-day life so a long-idle grant is re-consented before it can
...
export async function handleGoogleCallback(request: Request, env: Env): Promise<Response> {
  ...
  if (!tok.refresh_token) {
    // prompt=consent should always return one; if Google still withholds
    // it the session would be unrefreshable — fail loudly rather than
    // mint a cookie that dies in an hour.
...
// A live Google access token for one Drive call, refreshing transparently
// when it's within 60s of expiry. When it refreshes it returns a
// `setCookie` header the caller MUST attach to its own Response (the
// re-encrypted session). `null` → no session, or the refresh token is
// dead (revoked / 6-month idle / password change) → the caller 401s and
// the client prompts a reconnect.
