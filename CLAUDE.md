...
`Workspace` (`stores/workspaces.ts`) is the sharing unit, not the document — one active workspace at a time (VS Code-style), documents (`stores/docs.ts`) scoped to it and movable between workspaces. Sharing a workspace links it to a `WorkspaceRoom` Durable Object (`src/workspace-room.ts`, one instance per workspace id) that holds every document's live `Y.Doc` plus non-Yjs metadata (access record, version snapshots) in DO storage, relayed over **one WebSocket per workspace** (not per document) — every `MESSAGE_SYNC`/`MESSAGE_AWARENESS` frame is prefixed with a `docId` string so the single connection can multiplex all of a workspace's documents at once. `MESSAGE_PRESENCE` frames carry cross-document "who's looking at what" separately from any one document's own y-protocols `Awareness` instance.

`CollabRoom` (`src/collab-room.ts`) is now a **migration-only shim** — the pre-workspace one-DO-per-document design, reduced to `POST /api/collab/<id>/migrate` (auth-gated, idempotent via a `migratedTo` tombstone) which seeds a fresh `WorkspaceRoom` via `/internal/seed` from whatever was last checkpointed in the legacy room; every other legacy endpoint is gone and `fetch` 410s them. `collab.ts`'s `migrateLegacyDoc` calls `/migrate` on open, then talks only to `WorkspaceRoom`. New work targets `WorkspaceRoom`; `CollabRoom` stays only until every legacy link has been opened once (spec `docs/superpowers/specs/2026-09-10-collabroom-migration-shim-design.md`).

A document's per-document `Y.Doc` (client side: `DocBinding` in `collab.ts`) carries more than just its text — `ytext` (content), `imagesMap` (`Y.Map<string>`, pasted/dropped images), `meta` (`Y.Map<string>`, currently just `name`), `suggestions` (`Y.Map<SuggestionEntry>`, tracked-change proposals), and `comments` (`Y.Map<CommentThreadEntry>`, comment threads — moved off HTTP storage into the doc in v1.62.0) are top-level types on the same doc, all riding the same sync/persistence/broadcast path for free; adding another synced field on an existing shared document is "add another top-level type here," not new server code. Every write to the Y.Doc is gated editor-or-reviewer server-side (`WorkspaceRoom.handleMessage`'s `isWrite` check drops viewers) and then role-validated per top-level type by an **observer that reverts a disallowed write** in a dedicated transaction origin the observer itself skips: `suggestions` — a reviewer's raw text deletes / entry deletes / self-accepts are reverted (`reviewer-integrity.ts`, `"suggestion"` origin); `comments` — author/transition rules (`comment-integrity.ts`, `"comment-reconcile"` origin). Version restores are a separate, more permissive HTTP endpoint (`viewer`/`reviewer`/`editor`).
...
A `Co-Authored-By` trailer on a commit is fine. Never append a `Claude-Session: https://claude.ai/code/...` link (or any other link back to an internal Claude session) to a commit message or PR description — that URL is private to whichever session created it and has no value in this repo's public history.
