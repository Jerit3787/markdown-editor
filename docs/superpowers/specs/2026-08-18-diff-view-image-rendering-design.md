...
- `Snapshot` gains `images?: Record<string, string>`. `appendSnapshot(docId, content, now, images)` and `maybeSnapshotVersion(docId, content, images, now?)` take the new parameter; old stored records simply lack the field (no migration needed — `IDBObjectStore` has no schema to migrate, missing keys just read as `undefined`).
...
- `restoreLocalVersionContent(docId, content, now?)` (used for repo-commit restores, where there's no existing local snapshot to look up) gains an `images` parameter — `restoreLocalVersionContent(docId, content, images, now?)` — passed straight through to its own internal `appendSnapshot` call. `VersionHistory.svelte`'s `restore()` passes `selectedImages` here for the repo-commit branch.
...
- `maybeSnapshot`/`forceSnapshot` extract `docRoom.doc.getMap<string>("images")` into a plain object (`Object.fromEntries(map.entries())`) at snapshot time — the data is already present via normal Yjs sync (`client/src/collab.ts`'s `createDocBinding` creates this same `images` Y.Map client-side and it replicates like any other Yjs type); this is a read of already-synced state, not new sync plumbing.
- The versions-list/version-content HTTP handlers include `images` in their JSON response.
- `getSharedVersionContent` (client) is joined by `getSharedVersionImages`, mirroring Phase 1's split. `restoreSharedVersion`/`restoreSharedVersionContent` — restoring a shared version already propagates through the normal Yjs sync channel (per `history.ts`'s existing comment on `restoreSharedVersion`); the server-side restore handler needs to also write the snapshot's `images` back into the Y.Map, same reasoning as Phase 1's restore fix.
- `VersionHistory.svelte`: `selectVersion`'s shared branch also calls `getSharedVersionImages`.
