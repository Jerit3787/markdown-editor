...
- **No version bump / CHANGELOG / whats-new in this plan.** The feature isn't user-visible-complete until plan 3+ (the spec ships it all as one `1.49.0` release). `CHANGELOG.md` may gain a provisional `## [1.49.0]` `### Added` bullet that later plans extend.
...
- **Modify `client/src/app.ts`** — extend the popup `message` listener to also handle `{ type: "mde-google-auth" }`.
...
**Files:** Modify `src/auth.ts`, `src/github-auth.ts`
**Test:** existing `tests/src/github-auth.test.ts` must stay green (pure refactor).
...
- [ ] **Step 1: Write the failing test** — a new `tests/src/auth.test.ts`:
...
describe("encryptJSON / decryptJSON", () => {
  it("round-trips an arbitrary object and stamps an exp", async () => {
    ...
    const back = await decryptJSON<{ refreshToken: string; accessToken: string; accessTokenExp: number }>(env, token);
  ...
  it("returns null once exp has passed", async () => {
    ...
    expect(await decryptJSON<{ x: number }>(env, token)).toBeNull();
  });
});
```
...
// that one at /api/auth/github/login). app.ts's message listener handles
// the { type: "mde-google-auth" } postMessage and calls
...
- [ ] **Step 4: `client/src/app.ts`** — find the existing `window.addEventListener("message", ...)` that handles `data?.type === "mde-github-auth"` (near the GitHub popup open). Extend it:

```ts
    if (data?.type === "mde-github-auth" && data.ok) window.MDE.onGithubAuthComplete?.();
    if (data?.type === "mde-google-auth" && data.ok) window.MDE.onGoogleAuthComplete?.();
```

(Match the exact existing shape — it may `if (event.origin !== window.location.origin) return;` first; keep that.)
...
Find where `Settings.svelte` renders the GitHub sign-in / sign-out control (it uses `githubUsername`). Add a sibling "Google Drive" row:
...
At the top of `CHANGELOG.md`, above `## [1.48.8]`:
...
(`- UNRELEASED` heading + no version bump: this is edited across plans 1–5, then the version/date/whats-new go in on the final plan, per the spec and CLAUDE.md's "don't bump while implementing".)
...
git push -u origin feat/google-drive-sync
```

PR against `master`, title `feat(drive): connect Google Drive + open markdown from Drive (plan 1/5)`, body summarising the connection + import flow, the `drive.file` scope choice, the Picker security note, and that this is the first of five plans toward one `1.49.0` release. Wait for CI green; **do not merge yet** unless the whole feature is meant to ship incrementally — confirm with the user (per the workspace-pivot pattern, plans on one feature branch may accumulate before a single release).
...
- Spec "no CSP currently exists" (verified during planning — `grep` found no `Content-Security-Policy` anywhere) → Task 10 loads `apis.google.com` with no CSP change needed; the spec's open-question 4 is resolved as "nothing to do".
...
- Popup message `type: "mde-google-auth"` — set in `popupHtml` (Task 2), matched in `app.ts` (Task 9).
