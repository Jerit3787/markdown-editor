...
`joinSharedLink` itself remains untested (async, DOM/module-state-coupled orchestration — same boundary the codebase already draws around `openShareModal`, which calls `decideShareTarget` but is itself untested). The wiring change is verified live in a browser: seed an existing workspace, simulate a single-doc share link resolving via a stubbed `fetch`, confirm no modal appears and the new workspace is created with the doc's real name and lands as active; then confirm a multi-doc share link still shows the modal unchanged.
