...
- `restoreSharedVersion`/`restoreSharedVersionContent` — restoring a shared version already propagates through the normal Yjs sync channel (per `history.ts`'s existing comment on `restoreSharedVersion`); the server-side restore handlers (`handleVersionRestoreRequest`/`handleVersionRestoreContentRequest`) need to also write the snapshot's `images` back into the `images` Y.Map, inside the same `doc.transact(...)` block that already restores the text — a full replace (delete every existing key, then set each of the snapshot's), mirroring Phase 1's `replaceDocImages` semantics, not a merge.
