...
> **Re-validated 2026-09-11** against master at v1.64.1 (the original spec
> was written at ~v1.45 and the paused branch `feat/google-drive-sync`
> / PR #172 is ~295 commits behind — close it, do not merge). Everything
> the spec builds on is intact: `repo-sync.ts`'s planners, the
> `github-auth.ts` popup + `src/auth.ts` crypto, `worker.ts` routing,
> `stores/repoSync.ts` + `RepoConflictModal.svelte`, the `MenuBar.svelte`
> File-menu submenus, `mergeSnapshotsFromRepo`. Five things moved — see
> the **Re-validation deltas (2026-09-11)** notes inline and the CSP
> section, now rewritten. Version target is **1.65.0** (was 1.49.0). The
> paused branch's new self-contained files (`src/google-auth.ts`,
> `src/google-drive.ts`, `client/src/drive-files.ts`,
> `client/src/stores/driveSync.ts` + their tests) are ~90% reusable as
> Plan 1 starting points.
...
> **Re-validation deltas (2026-09-11).** All still present. Line numbers
> are stale — find by name. `popupHtml`'s `postMessage` type is now
> `"mde-github-auth"` (not `"github-auth"`); the Google copy uses
> `"mde-google-auth"`. `src/auth.ts` now also exports `signAnonToken` /
> `verifyAnonToken` and `toBase64Url` accepts `ArrayBuffer | Uint8Array` —
> the `encryptJSON` / `decryptJSON` extraction must leave those untouched.
> `Settings.svelte` is opened from the topbar account menu via
> `settingsModalOpen` (v1.56.0), not its own button.

- **`src/github-auth.ts`** — `handleLogin` builds the GitHub auth URL +
  a state cookie and returns a redirect; the flow runs in a **popup**
  (`client/src/app.ts` `window.open("/api/auth/github/login", …)`),
  whose final page is `popupHtml()` — it `postMessage`s
  `{ type: "mde-github-auth", ok }` to `window.opener` at
...
- **Version History ↔ repo commits (v1.24.0)** —
  ...
  work against them. **Re-validation delta (2026-09-11):** `HistoryEntry`
  is now `LocalEntry | CommitEntry | SessionEntry` (a `SessionEntry` for
  live-collab session grouping was added since). Plan 2's
  `commit → remote` generalization must be additive — it must not clobber
  `SessionEntry`.
- **`src/worker.ts`** routes `/api/auth/github/{login,callback,logout,me}`
  and `/api/repo/*`, `/api/gist*`, `/api/workspace/*`, `/api/collab/*` —
  a mix of exact `url.pathname === "…"` checks and `const X_PATH = /re/`
  + `url.pathname.match(X_PATH)`. Google routes slot in the same way.
...
anyway) and can be plain `wrangler.jsonc` `vars`. Local: git-ignored
...
### Content-Security-Policy — TWO definitions to update (2026-09-11)

Since v1.60.3 the app's CSP is **per-request, Worker-set** with a fresh
nonce: `src/csp.ts` `appCsp(nonce)` builds the `Content-Security-Policy`
response header for every HTML response (`src/worker.ts` calls it). The
`<meta http-equiv="Content-Security-Policy">` in `client/index.html` is
kept as a **dev-only** policy (Vite dev server has no Worker). **Both
must change** — and stay consistent — for the Picker + Drive API:

| Directive | add | why |
|---|---|---|
| `script-src` | `https://apis.google.com` | the Picker loader `https://apis.google.com/js/api.js` |
| `frame-src` | `https://docs.google.com` | the Picker renders in a `docs.google.com` iframe |
| `connect-src` | `https://www.googleapis.com` | `getGoogleAccessToken` is server-side, but the client Picker's own XHRs and any client Drive fetch hit `googleapis.com` |
| `img-src` | already `https:` — no change | Picker thumbnails |

`src/csp.ts` has a `legalCsp(nonce)` too (the `/privacy` `/terms`
pages) — leave it alone; those pages never touch Drive. The plan adds a
`tests/src/csp.test.ts` assertion (or extends the existing one) that
`appCsp` contains the three Google origins, and the built-bundle
collab e2e (`tests/e2e/collab/csp-built.spec.ts`) still passes.
...
- **Picker load**: lazy-load Google's picker from
  `https://apis.google.com/js/api.js` (needs the CSP additions — see the
  **Content-Security-Policy** section above; both `src/csp.ts` and the
  dev `<meta>`), then `gapi.load("picker", …)`. `GET /api/auth/google/picker-token` supplies
...
- **`HistoryEntry`** union (now `LocalEntry | CommitEntry | SessionEntry`
  — see the re-validation delta) → generalize `CommitEntry` into a
  `RemoteEntry` with `source: "github" | "drive"`; **`SessionEntry` and
  `LocalEntry` are untouched**. `loadCommitEntries` → `loadRemoteEntries`
  with a `source` param. `VersionHistory.svelte`'s
...
User-facing → **minor bump** — **`1.65.0`** (revised 2026-09-11 from
`1.49.0`; master is at 1.64.1). All three: `package.json` +
...
4. **Picker CSP — RESOLVED (2026-09-11), see the Content-Security-Policy
   section.** Two files: `src/csp.ts` `appCsp()` (prod, per-request
   nonce) and `client/index.html` `<meta>` (dev). Add `apis.google.com`
   to `script-src`, `docs.google.com` to `frame-src`, `www.googleapis.com`
   to `connect-src`.
...
7. **`dev-login.patch` fragility (2026-09-11).** Plan 1 adds imports and
   route consts to the top region of `src/worker.ts` — the exact region
   `tests/scripts/manual-testing/dev-login.patch` anchors on. That patch
   (applied by the `e2e-collab` / `e2e-github` CI jobs) **will fail to
   apply** afterwards. Plan 1's final task **must** regenerate it: apply
   the worker.ts changes, hand-apply the 3 dev-login hunks, `git diff
   src/worker.ts > …/dev-login.patch`, `git checkout src/worker.ts`,
   verify `git apply --check` + reverse both clean. Also keep
   `scripts/check-no-dev-login.mjs` green (it scans `src/**/*.ts` for
   `/api/dev/login`).
6. **Plan decomposition.** One feature, large (new OAuth provider + Drive
   ...
   files). Ships as one release (`1.65.0`); the *implementation plan* is
