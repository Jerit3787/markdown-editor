...
import { diagramKey, resolveDiagramRefs } from "../../../client/src/diagram-refs";
...
describe("resolveDiagramRefs", () => {
  it("resolves a known ref to its stored source", () => {
    ...
    expect(resolveDiagramRefs(text, diagrams)).toBe("# Title\n\n```mermaid\nflowchart TD\nA-->B\n```\n");
  });
  ...
  it("resolves multiple diagrams in the same document independently", () => {
    ...
    expect(resolveDiagramRefs(text, diagrams)).toBe("```mermaid\nflowchart TD\nX-->Y\n```\n\ntext\n\n```mermaid\nflowchart LR\nP-->Q\n```\n");
  });
