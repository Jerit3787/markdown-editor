...
- A new Playwright test in `tests/e2e/local` (this suite already covers exactly this class of client-only interaction — formatting, export, focus mode): open the bar via keyboard shortcut, type a query, assert the match count and at least one visible highlight, replace one occurrence, Replace All the rest, assert final document content.
- No Svelte component unit tests, matching this codebase's established precedent (no component-level test harness exists here).
